    ...
    <div class="main-card mb-3 card">
    	<form method="POST" action="{{ route('admin.users.update',$user->id) }}"> 
	        ...
	        <div class="card-header">
                {!! editButton(route('admin.users.index'),'Back','btn-xs btn-secondary mx-3 back-btn','lnr-pointer-left') !!}
                            ...
                            <select name="fm" class="form-control fm @error('fm') is-invalid @enderror">
                                ...
                                @if($fundManagers)
                                    @foreach($fundManagers as $current_fm)
                                        @php 
                                        $selectedFM = old('fm') ?? $fm ?? '';
                                        $selected = $current_fm->id==$selectedFM?'selected="selected"':''; 
                                        @endphp
                                        <option value="{{ $current_fm->id }}"  {{$selected}}>{{ $current_fm->name }}</option>
                                    @endforeach
                                @endif
                            </select>
                        </div>
	        ...
	        <div class="card-footer">
	            <button class="btn-shadow btn-wide btn-pill btn-hover-shine btn btn-primary">Update User</button>
	        </div>
        </form>
    </div>
</div>
